...
def generate_csrf_token() -> str:
    ...
    CSRF (Cross-Site Request Forgery) tokens prevent attackers from
...
def register():
    ...
    if request.method == "POST":
        ...
        if not verify_csrf_token(csrf_token):
            ...
            # Keep form values even on CSRF error
    ...
    # Escape username and email to prevent XSS attacks
    ...
    page_content = f"""
        <form method="post">
          ...
          <label>Username
            <input type="text" name="username" required minlength="3" maxlength="32" placeholder="e.g. Kakashi" value="{username_escaped}">
          </label>
          
          <label>Email
            <input type="email" name="email" required placeholder="your.email@example.com" value="{email_escaped}">
          </label>
          <label>Password
